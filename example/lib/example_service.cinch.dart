...
class _$ExampleService extends gg.Service {
  ...
  Future<Global<List<Example>>> _$getArea3(String? type) {
    return request(<dynamic>[
      ...
      (const gg.Query('type'), type)
    ]).then((dynamic response) => Global<List<Example>>.fromJson(
        response.data,
        (json) => List<Example>.from(
            (json as List).map((j) => Example.fromJson(j)))));
  }
  ...
  Future<Global<List<String>>> _$getArea4(String? type) {
    return request(<dynamic>[
      ...
      (const gg.Query('type'), type)
    ]).then((dynamic response) => Global<List<String>>.fromJson(response.data,
